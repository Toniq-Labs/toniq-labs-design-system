import {AnonymousIdentity, type Identity, type SignIdentity, type Signature} from '@dfinity/agent';
import {
    Delegation,
    DelegationChain,
    DelegationIdentity,
...
export class DelegationSignerClient extends SignerClient {
    ...
    public static async create(
        ...
        if (!baseIdentity) {
            const createdBaseIdentity = await (!options?.keyType ||
    ...
    public async login(options?: {
        ...
        >({
            ...
            params: {
                ...
                maxTimeToLive:
                    options?.maxTimeToLive === undefined ? '' : String(options.maxTimeToLive),
            },
        });
