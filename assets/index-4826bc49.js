...
`,de=Ln()({tagName:"vira-icon",hostClasses:{"vira-icon-fit-container":({inputs:e})=>!!e.fitContainer},styles:({hostClasses:e})=>h`
    ...
    `,events:{expandChange:ie()},stateInitStatic:{contentHeight:0},renderCallback({state:e,updateState:t,dispatch:n,events:r,inputs:o}){const i=o.expanded?h`
        ...
        `}});const O=yt({"vira-icon-stroke-color":"currentColor","vira-icon-fill-color":"none","vira-icon-stroke-width":"1px"});function v({name:e,svgTemplate:t}){return{name:e,svgTemplate:t}}const Zm=v({name:"CloseX24Icon",svgTemplate:c`
...
`,Bd=v({name:"LoaderAnimated24Icon",svgTemplate:c`
    ...
    `,renderCallback({inputs:e,dispatch:t}){return c`
            ...
            <input
                ...
                ${k("input",async n=>{const r=n.currentTarget;if(!(r instanceof HTMLInputElement))throw new Error("Failed to find input element for search.");const o=r.value;await ca(200),r.value===o&&(r.value?t(new No({paths:[Me.Search,encodeURIComponent(r.value)]})):t(new No(Tn)))})}
            />
...
`))}const B=(()=>{const e=h`
    ...
    `,x=h`
        ${Se("rubikFont")}
    `;return{toniqFont:e,normalWeightFont:t,boldFont:n,extraBoldFont:r,paragraphFont:o,boldParagraphFont:i,labelFont:a,boldLabelFont:s,h1Font:l,h2Font:u,h3Font:p,h4Font:f,h5Font:m,h6Font:g,monospaceFont:b,rubikFont:x}})(),Ar={popupShadow:h`
            ...
            `}function b($){i({internalValue:$}),e.disabled||r(new n.valueChange($))}function x($,A,S){const D=Vt((m-Wt)*(S.max-A)/(S.max-S.min)+Wt/2),R=ea($,u,l),G=_i({value:A,limits:S,flip:!1});return c`
                ...
                >
                    ...
                    <input
                        ?disabled=${e.disabled??!1}
                        type="range"
                        class="${L.slider} ${e.colorTemperature?t5(Math.abs(G)):""}"
                        step=${e.step}
                        .min=${S.min}
                        .max=${S.max}
                        .value=${A}
...
`;function tb(e,t,n){const r=Mh(n);return e({title:t,styles:eb,renderCallback(){return c`
            ...
            `}})}const nb=U({title:"Fonts",parent:ui,elementExamplesCallback({defineExample:e}){ce(B).map(t=>{const n=B[t];return tb(e,t,n)})},descriptionParagraphs:['Accessed via the "toniqFontStyles" export.']}),rb=U({title:"Shadows",parent:ui,elementExamplesCallback({defineExample:e}){Nh(Ar).forEach(([t,n])=>{e({title:t,styles:h`
                        div {
                            width: 50px;
                            height: 50px;
                            border-radius: 8px;
